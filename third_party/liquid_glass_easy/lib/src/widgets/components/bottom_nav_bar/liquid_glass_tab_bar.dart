...
class LiquidGlassTabBar extends StatelessWidget {
  const LiquidGlassTabBar({
    ...
    int? centerGapAfter,
        ...
        centerGapAfter = centerGapAfter ??
            (items.length > 2 ? (items.length ~/ 2) - 1 : 0),
  ...
  /// Width of a **slot-free span** in the middle of the bar, in tab units
  /// (`1` = one tab cell). `0` (the default) keeps upstream's evenly
  /// split row.
  ///
  /// This is the vendored addition the docked centre action needs: it
  /// reserves a hole between tab [centerGapAfter] and the next tab, so a
  /// raised action button can sit in the middle of the bar instead of on
  /// top of a tab. The tabs keep their cell width and the bar simply
  /// gives up that span; the moving selection pill still travels between
  /// tab centres, so it crosses the gap at a constant rate.
  ///
  /// Indices ([items], [selectedIndex], [onChanged]) are unaffected —
  /// they stay contiguous tab numbers and never see the gap.
  final double centerGap;

  /// Tab the [centerGap] is inserted after. Defaults to the middle of the
  /// row, which on a four-tab bar is tab `1`.
    ...
    if (_impellerStandalone) {
      ...
      if (ui.ImageFilter.isShaderFilterSupported) {
        return buildGlassPillBar(
          ...
          // This overlay has no sibling capsules to share a backdrop read
          // with. On Impeller GLES the composed batch blur/shader path
          // can lose the capsule entirely; separate passes keep its frost
          // and tint visible over the live page. The moving pill already
          // owns a separate read.
          batch: false,
        );
      }
