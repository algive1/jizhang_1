...
const feature = z.enum(['export', 'summary', 'voice']);
export const assistantPolicySchema = z.strictObject({
  ...
  memberFeatures: z.array(feature).max(3).refine(v => new Set(v).size === v.length),
});
...
export function registerAssistantPolicy(
  ...
  const authorize = (userId: string, body: AssistantRequest): AuthorizationResult => {
    ...
    return store.db.transaction(() => {
      ...
      const member = !!store.db.prepare('SELECT 1 FROM assistant_memberships WHERE user_id=? AND expires_at>?').get(userId, now);
