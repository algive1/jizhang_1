...
export const catalogSchema = z.strictObject({
  ...
  benefits:z.array(z.strictObject({id:z.enum(['automatic','books','statistics','cloud','assets','export','theme','support']),title:text,subtitle:text,detail:text})).length(8).refine(p=>new Set(p.map(v=>v.id)).size===8),
