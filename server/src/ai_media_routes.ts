...
export function registerAiMediaRoutes(app:FastifyInstance,store:Store,authenticate:Authenticate){
  app.post('/api/v1/ai/asr',{config:{rateLimit:{max:20,timeWindow:'1 minute'}}},async req=>{
    const user=authenticate(req.headers.authorization);consumeEntitlement(store,user.id,'voice_bookkeeping',1);const mime=String(req.headers['content-type']??'audio/m4a').split(';')[0]!;
  ...
  app.post('/api/v1/ai/ocr',{config:{rateLimit:{max:30,timeWindow:'1 minute'}}},async req=>{
    const user=authenticate(req.headers.authorization);consumeEntitlement(store,user.id,'ocr_import',1);const mime=String(req.headers['content-type']??'image/jpeg').split(';')[0]!;
