...
import { checkEntitlement, consumeEntitlement } from './entitlement_usage.js';
...
export function registerAiMediaRoutes(app:FastifyInstance,store:Store,authenticate:Authenticate){
  app.post('/api/v1/ai/asr',{config:{rateLimit:{max:20,timeWindow:'1 minute'}}},async req=>{
    const user=authenticate(req.headers.authorization);checkEntitlement(store,user.id,'voice_bookkeeping',1);const mime=String(req.headers['content-type']??'audio/m4a').split(';')[0]!;
    if(!mime.startsWith('audio/'))throw new ApiError(415,'仅支持音频内容');
    const result=await transcribeAudio(store,{userId:user.id,bytes:bodyBuffer(req.body,20*1024*1024),filename:String(req.headers['x-filename']??'voice.m4a').slice(0,120),mime});consumeEntitlement(store,user.id,'voice_bookkeeping',1);
  ...
  app.post('/api/v1/ai/ocr',{config:{rateLimit:{max:30,timeWindow:'1 minute'}}},async req=>{
    const user=authenticate(req.headers.authorization);checkEntitlement(store,user.id,'ocr_import',1);const mime=String(req.headers['content-type']??'image/jpeg').split(';')[0]!;
    if(!mime.startsWith('image/'))throw new ApiError(415,'仅支持图片内容');
    const result=await recognizeImage(store,{userId:user.id,bytes:bodyBuffer(req.body,12*1024*1024),mime});consumeEntitlement(store,user.id,'ocr_import',1);
