...
export const schemas: Record<Kind, z.ZodType<Data>> = {
  ...
  receivables: z.strictObject({ ...common, ...dated, name, type: z.enum(['reimbursement','refund','lend','customer','other']), counterparty: name, total_amount_in_cents: cents.positive(), received_amount_in_cents: cents.nonnegative(), occurred_at: time, expected_at: time.nullable().optional(), status: z.enum(['pending','partial','completed','writtenOff','overdue']), business_status: text, remark: optionalText }),
