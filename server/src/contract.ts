...
export const kinds = ['accounts', 'categories', 'transactions', 'goals', 'goal_milestones', 'goal_contributions', 'budgets', 'recurring_bills', 'installment_plans', 'books'] as const;
...
export const schemas: Record<Kind, z.ZodType<Data>> = {
  ...
  recurring_bills: z.strictObject({ ...common, ...dated, name, type: z.enum(['membership','mortgage','rent','carLoan','insurance','subscription','mobilePlan','income','other']), amount_in_cents: cents.positive(), cycle: z.enum(['weekly','monthly','quarterly','halfYear','yearly','custom']), start_date: time, end_date: time.nullable().optional(), next_date: time, account_id: nullableId, category_id: nullableId, custom_interval_days: z.number().int().positive().nullable().optional(), auto_record: flag, reminder: flag, status: z.enum(['active','paused','ended']) }),
